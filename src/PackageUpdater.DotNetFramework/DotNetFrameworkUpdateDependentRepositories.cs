...
{
    public class DotNetFrameworkUpdateDependentRepositories : IUpdateDependentRepositories
    {
        private static Task<Repository> UpdateDependentRepository(UpdatePackageCommandInput input, Repository repository,
            Action<Repository> beforeUpdateRepository = null, Action<string> afterUpdateRepository = null, CancellationToken cancellationToken = default)
        {
            ...
            foreach (string dependenciesFile in repository.DependenciesFiles)
            {
                ...
                if (!input.WhatIfFlag)
                {
                    Project.UpdatePackagesConfigReference(dependenciesFile, input.PackageNameFlag, input.PackageVersionFlag);
